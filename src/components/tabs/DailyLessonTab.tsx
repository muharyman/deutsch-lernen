...
function PracticeCard({
  ...
  return (
    <div className="practice-card">
      ...
      {checked ? (
        <div className={`practice-feedback ${question.type === 'short_answer' ? (shortAnswerCorrect ? 'correct' : 'wrong') : ''}`}>
    ...
    <section className="animate-fade">
      <article className="card lesson-summary">
        <div className="lesson-summary-top">
          ...
          <span className={`source-pill ${source === 'gemini' ? 'gemini' : 'static'}`}>
            {source === 'gemini' ? 'Gemini free tier' : 'Fallback lokal'}
          </span>
        </div>
        ...
        {!hasApiKey ? (
          <p className="inline-note">
            Gemini API key belum diisi. App menampilkan materi fallback sampai key dimasukkan di tab Settings.
          </p>
        ) : null}
        {error ? (
